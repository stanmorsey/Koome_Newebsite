...
function filterProducts(category) {
  ...
  displayProducts(filteredProducts, document.getElementById('product-container'));
}
...
function applyPriceFilter() {
  ...
  if (priceFilter === 'low') {
    filteredProducts = allProducts.filter(product => parseInt(product.price.replace('$', '')) < 5000);
  } else if (priceFilter === 'medium') {
    filteredProducts = allProducts.filter(product => parseInt(product.price.replace('$', '')) >= 5000 && parseInt(product.price.replace('$', '')) <= 15000);
  } else if (priceFilter === 'high') {
    filteredProducts = allProducts.filter(product => parseInt(product.price.replace('$', '')) > 15000);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (query) {
    fetchProducts().then(() => {
      ...
      displayProducts(filteredProducts, document.getElementById('searchResults'));
    });
  }
});
